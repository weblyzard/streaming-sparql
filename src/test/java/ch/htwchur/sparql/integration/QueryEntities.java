 ...
 *
 * <p>This client has been developed to test the performance of SPARQL queries by retrieving data on
 * resources specified in an entity file from a linked data repository.
 *
 * <p>The entities to query need to be encoded in a JSON encoded list.
 *
...
public class QueryEntities {
    ...
    public static void main(String[] args)
        ...
        if (args.length < 4) {
            ...
            System.out.println(
                    " Threads  ... number of parallel threads used to process the query");
            System.exit(-1);
        }
                                                ...
                                                ee -> {
                                                    ...
                                                    try (StreamingResultSet s =
                                                            StreamingQueryExecutor.getResultSet(
                                                                    url,
                                                                    QueryHelper.createRelationQuery(
                                                                            ee))) {
                                                        while (s.hasNext()) {
                                                            s.next();
